...
# setwd('C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/Data/') 
#CHANGE ME
setwd('C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data')
...
path_raw_F <- 'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/crypto_raw_forward-20240904T204820Z-001/crypto_raw_forward' # #CHANGE ME
path_raw_R <- 'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/crypto_raw_reverse-20240904T204823Z-001/crypto_raw_reverse'#CHANGE ME
...
sample.names <- sapply(strsplit(basename(fastqsF), '_R1'), `[`, 1) # CHANGE ME. As written, assumes filename = samplename_XXX.fastq.gz, splits the string based on the first "_" it finds, and takes chunk 1
...
plotQualityProfile(fnsF[[9]])
beep() # change the number in the brackets to look at different forward sample. Number is order of how they were loaded, not your sample number

plotQualityProfile(fnsR[[10]])
beep() # change the number in the brackets to look at different reverse sample. Number is order of how they were loaded, not your sample number
...
plotQualityProfile(fnsF, aggregate = T)
beep() # aggregates all forward reads/read1

plotQualityProfile(fnsR, aggregate = T)
beep() # aggregates all reverse reads/read2
...
rqc(
  path = "C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/crypto_raw_forward-20240904T204820Z-001/crypto_raw_forward",
  #CHANGE ME
  sample = TRUE,
  n = 1e+06,
  group = NULL,
...
DNA_to_read <- readDNAStringSet("Mock_S50_ExtNeg1_for.fastq",
                                format = "fastq",
                                with.qualities = TRUE)
...
### Summary of quality ----
# CHANGE ME: This dataset contains 23 number of samples, and 3,610,171 (paired forward/reverse) reads which are 300 bases long
# CHANGE ME: The R1 quality goes below 35 at cycle 175 but going to trim at 275 anyway. Yes the quality is good enough to use all R1.
# CHANGE ME: The R2 quality goes below 35 at cycle 200 but going to trim at 275 anyway. Yes the quality is good enough to use all R2.
...
dir.create(
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/filtered'
) #CHANGE ME
dir.create(
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/filtered/filtered_forward'
) #CHANGE ME
dir.create(
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/filtered/filtered_reverse'
) #CHANGE ME

# 2. Specify the folder path to place filtered files for reads 1 and 2.
path_filt_F <- 'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/filtered/filtered_forward' #CHANGE ME
path_filt_R <- 'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/filtered/filtered_reverse' #CHANGE ME
...
filtoutput <- filterAndTrim(
  file.path(path_raw_F, fastqsF),
  # path to and file names of the raw reads
  file.path(path_filt_F, paste0(sample.names, "_F_filt.fastq.gz")),
  # set filtered file paths and filtered file names to create
  trimLeft = 10,
  # cuts off the first XX bases from the F reads. Trim 10 for Illumina, 15 for 454 pyrosequencing.
  trimRight = 25,
  # cuts of last XX bases, or hash this out and use truncLen
  ...
  maxEE = 2,
  # the maximum number of expected errors allowed in a read, always >1
  maxN = 0,
  # max number of ambiguous bases (N) allowed, DADA2 doesn't allow any!!
  rm.phix = TRUE,
  # remove any PhiX DNA (used as positive control),
  verbose = TRUE,
  matchIDs = TRUE
)
beep() # verbose = print to screen
...
filtoutput <- filterAndTrim(
  ...
  fnsR,
  filtRs,
  # specifies in/out variables
  trimLeft = c(10, 10),
  # cuts off the first XX bases from the F,R reads. Trim 10 for Illumina, 15 for 454 pyrosequencing.
  trimRight = c(25, 25),
  # cuts of last XX bases from F,R reads, or hash this out and use truncLen
  ...
  maxEE = c(2, 3),
  ## The maxEE parameter sets the maximum number of expected errors allowed in a read. Always >1
  maxN = 0,
  # max number of ambiguous bases (N) allowed, DADA2 doesn't allow any!!
  rm.phix = TRUE,
  # remove any PhiX DNA (used as positive control)
...
# save the filtered variable for sample totals.  Your filtered files are already saved, this saves the summary R item to work with later without filtering again so you can track read numbers at each step.
saveRDS(filtoutput, "filtered_output_crypto.rds")
...
#Reproducing quality plots of filtered reads. This will help me determine if my parameters and cutting were effective in increasing overall quality.
...
### Summary of filtering ----
# CHANGE ME: Which parameters did you use and why?
  #trimLeft was used to cut off the first 10 bases for all reads (F & R). This is to avoid inclusion of the primer and to exclude poor quality bases.
  #trimRight was used to cut off the last 25 bases for all reads (F & R). This is because the quality score begins to crash at around 275 bases for both F & R reads (falling below 25--> TRASH)
  #maxEE was used to determine the expected error limit. For the F reads, 2 was chosen. For R, 3 was chosen (4, or 5 would be acceptable, but the R reads were decent enough quality to select 2)
  #maxN was set to 0 (dada2 only allows for this) to exclude any reads with ambiguous bases
  #rm.phix was set to TRUE to exclude any Phix DNA used as a positive control. This ensures that ONLY sample data is included in analysis. 
# CHANGE ME: What was the largest and smallest number of raw reads?
  #Largest: 40 @ 296378, Smallest: 37 @ 54609
# CHANGE ME: What was the largest and smallest number of filtered reads? 
  #Largest: 40 @ 237245, Smallest: 39 @ 47109
# CHANGE ME: How many total raw reads? 3610171
# CHANGE ME: How many total filtered reads? 2745757
...
saveRDS(errF, 'error_profile_F_CRYPTO.RDS') #CHANGE ME so 'example' is the name of your dataset
saveRDS(errR, 'error_profile_R_CRYPTO.RDS') #CHANGE ME so 'example' is the name of your dataset
...
errF <- readRDS('error_profile_F_CRYPTO.RDS')
errR <- readRDS('error_profile_R_CRYPTO.RDS')
...
dadaFs <- dada(filtsF, err = errF, multithread = FALSE)
beep()
dadaFs #To see SV count per sample
...
beep()
dadaRs #To see SV count per sample
...
dim(seqtab)
# CHANGE ME: 23 # of samples (first number, row count) and 9962# of SVs (second number, column count)

# 4. Save the seqtab before you quit!
saveRDS(seqtab,'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/seqtab_CRYPTO.rds') #CHANGE ME
...
seqtab <- readRDS(
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/seqtab_CRYPTO.rds'
) #CHANGE ME
...
seqtab.nochim <- removeBimeraDenovo(seqtab,
                                    ...
                                    multithread = FALSE,
                                    verbose = TRUE)
# CHANGE ME: My samples have 6578 bimeras out of 9962 total sequences

# 2. Check dimensions of your cleaned sequence table to see how many samples and sequences now remain.
dim(seqtab.nochim)
# CHANGE ME: 23 # of samples (first number, row count) and 3384# of SVs (second number, column count)
...
saveRDS(
  seqtab.nochim,
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/seqtab.nochim_CRYPTO.rds'
) #CHANGE ME
...
getN <- function(x)
  sum(getUniques(x)) #set function to count the unique reads
...
filtoutput <- readRDS("filtered_output_crypto.rds") #CHANGE file name
...
meta <- read.csv(
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/Crypto_bacteria_metadata.csv',
  # CHANGE file name
  header = TRUE,
  # specifies that the top row is column names
  row.names = 1
) # row.names specifies which column to call the samples by (sample name), doesn't have to be the first column
...
dim(filtoutput) # 23 rows (should match the number of samples)
dim(seqtab.nochim) # 23 rows (should match the number of samples)
dim(meta) # 23 rows (should match the number of samples)
...
track <- cbind(filtoutput, rowSums(seqtab.nochim), meta$Sample_source
...
colnames(track) <- c("reads.in", "filtered", "nonchimeras", "Sample_source") # CHANGE "Sample_type" to be the column name in your metadata file of the groups in your data
...
saveRDS(track, 'tracked_seqs_CRYPTO.RDS') #CHANGE file name to whatever you want
...
ggplot(plotData, aes(x = Sample_source, y = as.numeric(totals))) + geom_jitter(aes(color =
                                                                                   type)) + #CHANGE Sample_type to be the factor you previously chose
  ylab("Sequences") + xlab("Sample_source")

# or, plot with QA stage along the X axis
ggplot(plotData, aes(x = type, y = as.numeric(totals))) + geom_jitter(aes(color =
                                                                            Sample_source)) + #CHANGE Sample_type to be the factor you previously chose
...
unqs.NC <- seqtab.nochim["41", ] #CHANGE ME, update " " with sample name of a negative control, such as Mock_S13_PCRneg
...
cat(
  "DADA2 inferred",
  length(unqs.NC),
  "sequence variants present in the selected sample.\n"
)
# CHANGE ME: There are 106 SVs in this PCR negative control

# Plot the number of sequences in the SVs found in the negative control sample.
plot(unqs.NC,
     ylab = "Number of seqs/SV, Neg Control",
     xlab = "SVs",
     main = "Number of sequences/SV in Negative Control") #CHANGE ME
...
taxa.NC <- assignTaxonomy(
  unqs.NC,
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/R/silva_nr99_v138_train_set.fa.gz',
  minBoot = 75
)
write.csv(
  taxa.NC,
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/Data/taxa_silva_NC_EXAMPLE.csv'
)
...
plot(unqs.PC,
     ylab = "Number of seqs/SV, Neg Control",
     xlab = "SVs",
     main = "Number of sequences/SV in Positive Control") #CHANGE ME
...
write.csv(
  taxa.PC,
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/Data/taxa_silva_PC_EXAMPLE.csv'
)
...
## create a phyloseq object with all samples (subset out later). The otu_table is actually the SV table. Rename EX_ps to be what you want
EX_ps <- phyloseq(otu_table(seqtab.nochim, taxa_are_rows = FALSE),
                  sample_data(meta))

EX_ps
# how many samples made it this far? CHANGE ME, 23 samples, 11 variables, 3384 taxa
...
plot_richness(
  EX_ps,
  x = "Sample_source",
  # CHANGE the x-axis to a factor of your choice
  measures = c("Observed", "Chao1", "Shannon"),
  # these are some of the alpha diversity measures you can use
  color = "Sample_source"
) + # CHANGE the color to a factor of your choice
...
## SUMMARY: The negative controls are generally lower, but there is one in each measure that is right in the middle 
...
plot_ordination(EX_ps, EX.ord, type = "samples", color = "Sample_source")
...
EX_ps_controls = subset_samples(EX_ps, Sample_source == "Negative_control") # CHANGE ME to the column name that holds the variables associated with being a negative control

EX_ps_controls <- prune_taxa(taxa_sums(EX_ps_controls) > 0, EX_ps_controls)

#375 taxa in the 3 negative control samples
...
batch1 = subset_samples(EX_ps_NCclean, Batch_number == "1") #16 samples
...
batch1_kit = subset_samples(batch1, Sample_source == "Negative_control")
...
EX_ps_NCclean <- prune_taxa(taxa_sums(EX_ps_NCclean) > 0, EX_ps_NCclean)
EX_ps_NCclean <- prune_samples(sample_sums(EX_ps_NCclean) > 0, EX_ps_NCclean)
# 20 number of samples and 3009 SVs remaining CHANGE ME

# 10. Did it work? Check your ordination again
EX_cleaner.ord <- ordinate(EX_ps_NCclean,
                           #calculate similarities
                           method = "PCoA",
                           #ordination type
                           "jaccard",
                           binary = TRUE) #similarity type. Jaccard is binary, Bray can be binary (unweighted) or not (weighted)
...
plot_ordination(
  ...
  type = "samples",
  color = "Sample_source",
  title = "After cleaning out negative controls"
)

# save this graph for later

### Summary of decontamination ----
# CHANGE ME: Which types of negative controls did you use and why? The data set had DMEM and Negative_control labels, all removed
# CHANGE ME: Did you notice clustering by batch before and after removing contaminants? Only one batch in data, so doesn't apply. 
...
contamdf.freq <- isContaminant(EX_ps, method = "frequency", conc = "Extract_concentration_ng_ul") #CHANGE conc= "quant_reading" to the column heading that holds the concentration information in your metadata
...
# 6. How much is left
EX_ps.noncontamfreq
# CHANGE ME: 23 number of samples and 3379 SVs left
...
sample_data(EX_ps.noncontamfreq)$is.neg <- sample_data(EX_ps.noncontamfreq)$Sample_source == "Negative_control"

# 8. Calculate prevalence of SVs in samples versus controls
contamdf.prev05 <- isContaminant(EX_ps.noncontamfreq,
                                 method = "prevalence",
                                 neg = "is.neg",
                                 threshold = 0.5)
...
# CHANGE ME: 23 number of samples and 3273 SVs left


# 13. Did it work? Check your ordination again
EX_cleaner.ord <- ordinate(EX_ps_NC_decontam_clean,
                           #calculate similarities
                           method = "PCoA",
                           #ordination type
                           "jaccard",
...
# 1. reload as needed
seqtab.nochim <- readRDS(
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/Data/seqtab.nochim_EXAMPLE.rds'
)
...
all.taxa <- assignTaxonomy(
  seqtab.nochim,
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/R/silva_nr99_v138_train_set.fa.gz',
  # CHANGE file path
  tryRC = TRUE,
  # didn't get any IDs the first time? Use this to try the reverse complement of your sequences instead
  minBoot = 75,
  verbose = TRUE
)
beep()
...
write.csv(
  all.taxa,
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/Data/taxa_silva_EXAMPLE.csv'
) #CHANGE ME
...
seqtab.nochim.decontam = as(otu_table(EX_ps_NCclean), "matrix")
...
all.taxa <- assignTaxonomy(
  seqtab.nochim.decontam,
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/silva_nr99_v138.1_train_set.fa.gz',
  # CHANGE file path
  tryRC = TRUE,
  # didn't get any IDs the first time? Use this to try the reverse complement of your sequences instead
  minBoot = 75,
...
# 3. saveRDS(all.taxa, 'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/taxa_silva_CRYPTO.csv') #CHANGE ME
write.csv(
  all.taxa,
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/taxa_silva_CRYPTO.csv'
) #CHANGE ME
...
all.taxa.sp <- addSpecies(
  all.taxa,
  'C:/Users/micha/OneDrive/Documents/Bioinformatics Masters/DNA Sequencing Analysis Lab/Sample Data/silva_species_assignment_v138.fa.gz',
  allowMultiple = FALSE,
  verbose = FALSE
)
beep() #CHANGE file path

saveRDS(
  all.taxa.sp,
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/Data/taxa_species_silva_EXAMPLE.rds'
) #CHANGE ME
write.csv(
  all.taxa.sp,
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/Data/taxa_species_silva_EXAMPLE.csv'
) #CHANGE ME
...
meta <- read.csv(
  "C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/Data/Particle_Size_metadata.csv",
  header = T,
  row.names = 1
) #CHANGE ME
...
otu_t <- otu_table(EX_ps_NCclean)

## create a phyloseq object with all samples
EX_ps_NCclean <- phyloseq(otu_table(otu_t, taxa_are_rows = FALSE),
                                 sample_data(meta),
                                 tax_table(all.taxa))


saveRDS(EX_ps_NCclean, "CRYPTO_phyloseq_object_cleaned.rds")
...
df <- as.data.frame(EX_ps_NCclean)
table(df$Kingdom)
table(df$Phylum)
table(df$Class)
table(df$Order)
...
EX_ps_clean <- EX_ps_NCclean %>% # CHANGE ME to your phyloseq object name
  subset_taxa(Kingdom == "Bacteria" &
                Family != "Mitochondria" &
                Order != "Chloroplast") # CHANGE ME to taxa you want to remove
...
EX_ps_clean
# CHANGE ME: 20 number of samples and 2136 SVs left


# Save your clean phyloseq object
saveRDS(EX_ps_clean, 'CRYPTO_ps_clean_phyloseq_object.RDS') #CHANGE ME
...
EX-rarec <- rarecurve(tab, step = 10, cex=0.5, label = FALSE) 

 # step is major tick marks on x axis (x 1000), cex is text size, label is sample name
# optional to save this plot

# take a look at rowsums, or total sequences per sample
sort(rowSums(otu_table(EX_ps_clean)))
# CHANGE ME: smallest reads (sequences) in a sample: 1030
# CHANGE ME: largest in a sample: 79916
# CHANGE ME: number of samples with <5000: 7

EX_ps_clean.rar <- rarefy_even_depth(
  EX_ps_clean,
  sample.size = 2000,
  # CHANGE ME to the sequences/sample you want. 5-10k is a good amount, more is better
  replace = FALSE,
  #sampling with or without replacement
  trimOTUs = TRUE,
  #remove SVs left empty (called OTUs here but really they are SVs)
  rngseed = 711,
  verbose = TRUE
)

# CHANGE ME: how many: 3 samples and 1059 SVs were removed. Rarefaction cutoff of 2000.
#for training purposes and not for publication. This data set is small, so I did not want to lose
#too many samples. This DOES inflate the Kurtosis value later... 
...
write.csv(otu_table(EX_ps_clean.rar),
          'EX_ps_clean_rarefied_phyloseq_object.csv')
...
sample_data(EX_ps_clean.rar)$Diet <- factor(
  sample_data(EX_ps_clean.rar)$Diet,
  levels = c("LooseAlfalfa", "PelletedAlfalfa"),
  labels = c("Loose Alfalfa", "Pelleted Alfalfa")
) #CHANGE ME
...
plot_richness(EX_ps_clean.rar,
              x = "Sample_source",
              #CHANGE ME, A is whatever factor you want on x-axis
              measures = "Observed",
              #CHANGE ME, whatever alpha diversity measure you want. to have multiple, use: = c("Observed","Shannon")
              title = NULL) +
  theme_set(theme_minimal(base_size = 14)) + #make it look pretty
  theme(axis.text.x = element_blank()) + #example, get rid of x axis labels
  geom_violin(trim = TRUE, aes(fill = Sample_source)) + #optional. CHANGE ME, A is whatever factor to color violins
  geom_boxplot(width = 0.1, aes(group = Sample_source)) + #optional. CHANGE ME, A is whatever factor to group boxplots
  facet_grid(. ~ Sample_source, space = "free") +
...
plot1 <- plot_richness(EX_ps_clean.rar,
                       ...
                       #CHANGE ME, A is whatever factor you want on x-axis
                       measures = "Observed",
                       #CHANGE ME, whatever richness you want. = c("Observed","Shannon")
                       title = NULL) +
  theme_set(theme_minimal(base_size = 14)) +
  theme(axis.text.x = element_blank()) +
  geom_violin(trim = TRUE, aes(fill = Crypto_added)) + #optional. CHANGE ME, A is whatever factor to color violins
  geom_boxplot(width = 0.1, aes(group = Crypto_added)) + #optional. CHANGE ME, A is whatever factor to group boxplots
  # theme(legend.position = "none") + #use to get rid of your legend
  ylab("Observed Bacterial Richness (SVs)")
...
plot_richness(EX_ps_clean.rar,
  ...
  # theme(legend.position = "none") + #use to get rid of your legend
  ylab("Observed Bacterial Richness (SVs)") +
  ylim(0, 200) + #define the y axis min/max
  geom_segment(aes(
    x = 1,
    y = 185,
    xend = 2,
    yend = 185
  )) +  geom_text(x = 1.5, y = 185, label = "***") # add a drawn in line and significance tag, adjusting the x and y coordinates till it fits where you want in the window.  Add another for each line to add. As written, this will fit the view window you have, if you adjust that your segments will not adjust with it.
...
EX_ps_clean.rar.rich.df <- cbind(EX_ps_clean.rar.rich,
                                 EX_ps_clean.rar.even,
                                 EX_ps_clean.rar.sd)

dim(EX_ps_clean.rar.rich.df)

# make a graph using that dataframe. this is a generic example, you will want to personalize this.
ggplot(data = EX_ps_clean.rar.rich.df, aes(x = Sample_source, y = Observed)) +
  theme_minimal() +
  geom_point(aes(color = Sample_source), size = 3) + # sets colors by group and a set point size
  xlab("") +
...
ggplot(data = subset(EX_ps_clean.rar.rich.df, !is.na(FactorA)),
       aes(x = Sample_source, y = Observed)) +
  theme_minimal() +
...
EX_table = as.table(table(EX_ps_clean.rar.rich.df$Crypto_added, EX_ps_clean.rar.rich.df$Sample_source))
EX_table-----------# Write out your research questions, make them specific.
...
hist(EX_ps_clean.rar.rich.df$EX_ps_clean.rar.even)
hist(EX_ps_clean.rar.rich.df$Shannon)
#MINE WAS LEFT SKEWED. 
...
kurtosis(EX_ps_clean.rar.rich.df$EX_ps_clean.rar.even)
kurtosis(EX_ps_clean.rar.rich.df$Shannon)

# value output is 2.17565 (a VERY high kurtosis value, lots of outliers. could have been due to rarefaction value of 2000 rather than 5000), looking for whether +/- and if a large/small number
...
# W = 0.92083, p-value = 0.1525: my shannon diversity metric is not normally distributed (p-value > 0.05 reveals that the distribution of the data are not significantly different from normal distribution)

shapiro.test(EX_ps_clean.rar.rich.df$Observed)
#W = 0.9009, p-value = 0.07031, my observed data are not normal. : p-value > 0.05 reveals that the distribution of the data are not significantly different from normal distribution

shapiro.test(EX_ps_clean.rar.rich.df$EX_ps_clean.rar.even)
# W = 0.96741, p-value = 0.7716, not normally distributed. : p-value > 0.05 reveals that the distribution of the data are not significantly different from normal distribution
...
wilcox.test(Observed ~ Crypto_added, data = EX_ps_clean.rar.rich.df)
wilcox.test(EX_ps_clean.rar.even ~ Crypto_added, data = EX_ps_clean.rar.rich.df)
wilcox.test(Shannon ~ Crypto_added, data = EX_ps_clean.rar.rich.df)
...
fit <- aov(y ~ (W1 * W2 * B1 * B2) + Error(Subject / (W1 * W2)) + (B1 *
                                                                     B2), data = EX_ps_clean.rar.rich.df)
...
kruskal.test(Observed ~ Crypto_added, data = EX_ps_clean.rar.rich.df)
# data:  Observed by Diet
# Kruskal-Wallis chi-squared = 2.589, df = 1, p-value = 0.1076
...
conover.test(
  EX_ps_clean.rar.rich.df$Observed,
  EX_ps_clean.rar.rich.df$Week,
  method = c(
    "holm",
    "hochberg",
    "hommel",
...
) # method of correction, OK to pick just one
...
summary(lm(
  Observed ~ as.numeric(PHOS),
  data = subset(EX_ps_clean.rar.rich.df, Week == "2")
)) #CHANGE ME to update factor names or the alpha diversity metric you want
...
#CRYPTO HEAT MAP--> WITH AND WITHOUT CRYPTO ADDED
plot_heatmap(EX_ps_clean.rar.glom, fill = "Phylum", taxa.label = "Phylum") +   facet_grid(~ Crypto_added, space =
                                                                "free", scales = "free") + theme(legend.position = "bottom") #, axis.text.x = element_blank())
...
EX_ps_clean.rar.corr.df <- cbind(top15.sd,
                                 EX_ps_clean.rar.rich,
                                 EX_ps_clean.rar.even,
                                 EX_ps_clean.rar.sd)
...
EX_ps_clean.rar.corr.df <- subset(
  EX_ps_clean.rar.corr.df,
  select = -c(Project_name, DNA_extracted_by, DNA_extracted_date, Sample_source, Host_source, 
              Full_sample_name, Antibiotics, Crypto_added, Batch_number, Extract_concentration_ng_ul, 
              X260_to_280, X260_to_230)
)
...
# run correlations
ex_corr_calc <- cor(EX_ps_clean.rar.corr.df, use = "complete.obs", # use=Specifies the handling of missing data. Options are all.obs (assumes no missing data - missing data will produce an error), complete.obs (listwise deletion), and pairwise.complete.obs (pairwise deletion)
...
corrplot(
  ex_corr_calc,
  type = "lower",
  #shape of the plot itself: full, upper, lower
  method = "circle",
  #shape of the data: circle, square, ellipse, number, shade, color, pie
  order = "hclust",
  #how to cluster samples: AOE, hclust, FPC, alphabet, or leave blank
  #p.mat = res1[[1]],
  #which correlations to use
  sig.level = 0.05,
  #sets significance cutoff
  insig = "blank",
  #leaves p > 0.05 blank
  tl.col = "black",
  # text color
...
) #specify a color palette and number of shades needed
...
# example from class
EX_ps_clean.rar.glom = tax_glom(EX_ps_clean.rar, "Phylum")
plot_bar(EX_ps_clean.rar.glom, fill = "Phylum") +   facet_grid(~ Crypto_added, space =
                                                                 "free", scales = "free") + theme(legend.position = "bottom", axis.text.x = element_blank())


# make a stacked 100% bar chart in phyloseq

EX_ps_clean.rar.stacked = transform_sample_counts(EX_ps_clean.rar, function(x)
  x / sum(x))
...
core_taxa_standardW <- core_members(core_biomeW.rel,
                                    detection = 1 / 10000,
                                    prevalence = 50 / 100) #CHANGE ME

#A full phyloseq object of the core microbiota at those limits is obtained as follows
phylo.coreW <- core(core_biomeW.rel,
                    detection = 1 / 10000,
                    prevalence = .05)
...
write.csv(core.taxa.classW, "core.taxa.CRYPTO.csv")
...
relabun.ps <- transform_sample_counts(EX_ps_clean.rar, function(x)
  x / sum(x))
...
MySummary <- genus.df %>% group_by(Diet, Week, Genus) %>% summarize(mean_abund = mean(Abundance, na.rm =
                                                                                        TRUE))
...
diagdds = phyloseq_to_deseq2(EX_ps_clean, ~ Crypto_added) #CHANGE ME to an factor with only 2 levels
...
sigtab = sigtab[, c("baseMean",
                    ...
                    "Class",
                    "Family",
                    "Genus")] #CHANGE ME add Order or Species - if you have it
...
ggplot(sigtab, aes(y = Genus, x = log2FoldChange, color = Phylum)) + #play with aesthetics to make graph informative
  ...
  ) + xlab("log2 Fold Change") + labs(size = "Mean Sequence Abundance") + theme_minimal()
...
response <- as.factor(sample_data(EX_ps_clean)$Crypto_added) #CHANGE ME
...
response.pf <- rfPermute(
  response ~ . ,
  data = rf.data,
  na.action = na.omit,
  ntree = 500,
  nrep = 100
) #na.omit ignores NAs in data (not tolerated). ntrees is how many forests to build, nreps generates p-value
...
pred.abun.df$FactorA <- factor(pred.abun.df$FactorA, levels = c("no", "yes"), labels = c("Without_Crypto", "With_Crypto")) #CHANGE ME
pred.abun.df$FactorB <- sample_data(EX_ps_clean)$Sample_source #CHANGE ME


# optional, if you want factors to graph in a specific order, you can set that manually, and relabel them so they are more readable in the graph
pred.abun.df$Date <- factor(
  pred.abun.df$Date,
  levels = c("21_Apr", "12_May", "01_Jun", "22_Jun", "25_Jul"),
  labels = c("21 Apr", "12 May", "01 Jun", "22 Jun", "25 Jul")
) #CHANGE ME
...
m <- ddply(m, .(variable), transform, rescale = log(1 + as.numeric(value))) #NOTE: may need to change it to as.numeric(value) if it tells you an error about non-binary operators. if you go to graph it and it doesn't recognize 'rescale', it means this piece failed.
...
dim(predictors)
#CHANGE ME how many samples and SVs included
...
# Make one column for our outcome/response variable. Choose which one applies to the thing you want to test, and then follow factorial or numeric through the rest of the code section.
response.class <- as.factor(sample_data(EX_ps_clean)$Crypto_added) #CHANGE ME to the main factor you want to test
response.subclass <- as.factor(sample_data(EX_ps_clean)$Sample_source)
...
EX_clean.rar.pca <- prcomp(otu_table(EX_ps_clean.rar),
                           center = TRUE,
                           scale = TRUE) #CHANGE ME
...
EX_uJ_pcoa <- ordinate(EX_ps_clean.rar,
                       #calculate similarities
                       method = "PCoA",
                       #ordination type
                       "jaccard",
                       binary = TRUE) #similarity type. Jaccard is binary, Bray can be binary (unweighted) or not (weighted) but is usually run as binary=FALSE.

# simple ordination
plot_ordination(EX_ps_clean.rar,
                EX_uJ_pcoa,
                type = "samples",
                color = "Crypto_added") #CHANGE ME but leave type as "samples"
...
plot_ordination(EX_ps_clean.rar, EX_uJ_pcoa, type = "samples") + #CHANGE ME
  geom_point(aes(
    color = as.factor(Crypto_added),
    shape = as.factor(Sample_source),
    size = 1
  )) + # resize the points based on a numeric factor, and make light/dark based on another factor
  theme_minimal() +
  scale_color_viridis(
    discrete = TRUE,
    option = "A",
    begin = 0,
    end = 0.75) + #begin/end indicates where on the color scale to use, A refers to 'magma' color palette
  stat_ellipse(aes(group = as.factor(Sample_source), linetype = as.factor(Sample_source))) + #add circles around a particular grouping, and make the circle lines different
  labs(
    color = "Crypto_added",
    shape = "Sample_source",
    size = "Point Size",
    linetype = "Sample_source") #rename the headers in the legend


## NMDS in phyloseq -----------------

# use phyloseq to calculate ordination for use in the plot
EX_uJ_nmds <- ordinate(EX_ps_clean.rar,
                       #calculate similarities
                       method = "NMDS",
                       #ordination type
                       "jaccard",
...
plot_ordination(EX_ps_clean.rar,
  ...
  ) + #begin/end indicates where on the color scale to use, A refers to 'magma' color palette
  stat_ellipse(aes(group = Crypto_added, linetype = Crypto_added)) + #add circles around a particular grouping, and make the circle lines different
  labs(
    color = "Crypto_added",
    size = "Point Size",
    shape = "Sample_source",
    linetype = "Crypto_added"
  ) #rename the headers in the legend
...
adonis2(
  EX_uJ_nmds ~ Crypto_added + Sample_source,
  as(sample_data(EX_ps_clean.rar), "data.frame"),
  permutations = 9999,
  na.action = na.omit, by = "terms"
) #CHANGE ME so variable and factor reflect your data
# paste output here
...
adonis2(
  EX_uJ_nmds ~ Diet * Week * as.numeric(Weight_kg),
  data = data.frame(sample_data(EX_ps_clean.rar)),
  # CHANGE ME so 'Factor' is the name of your Factor variables
  permutations = ctrl,
...
EX_uJ <- vegdist(wisconsin(sqrt(EX_vegan)), method = "jaccard", binary =
                   TRUE) #CHANGE ME to be the method and weighted (binary = false) or unweighted version (binary = true)
...
betadisp_EX <- betadisper(EX_uJ, sampledf$Crypto_added) # CHANGE me so variable and factor reflect your data
...
cap_ord <- ordinate(
  ...
  #CHANGEME
  method = "CAP",
  #borrows capscale from the vegan package
  distance = bray_not_na,
  formula = ~ Crypto_added * Sample_source #CHANGE ME use for repeated measures
)
...
cap_plot <- plot_ordination(physeq = EX_ps_clean.rar,
                            #CHANGE ME
                            ordination = cap_ord,
                            axes = c(1, 2)) +
  theme_minimal() +
  aes(shape = as.factor(Sample_source)) + #CHANGE ME
  geom_point(aes(colour = Crypto_added), size = 4) + #CHANGE ME
  geom_point(colour = "grey90", size = 1.5)
...
# calculate CCA
cap.pcnm <- capscale(EX.df ~ ., data = as.data.frame(scores(EX.pcnm)), distance =
                       'bray')
...
mod0.pcnm <- capscale(EX.df ~ 1, data = as.data.frame(scores(EX.pcnm)), distance =
                        'bray') # add + Condition(SAMPLE_ID) for repeated measures
...
EX.pcnm.sub <- scores(EX.pcnm, choices = c(1, 3:16)) #CHANGE ME to include the significant pcnm numbers from previous command. If you want them all, write 1:16. to select only a few of them, write something like 1,3,6,7:9.

# partition variation among four predictor tables:
EX.var <- varpart(EX.df, ~ FACTORA, #CHANGE ME
                  ~ FactorB, #CHANGE ME
...
ex_meta$SourceSink <-  ifelse(ex_meta$Sample_source == "large_intestine_biopsy_culture", "source", "sink") #CHANGE ME
...
envs <- ex_meta$Sample_source # usually a factor that indicates location
desc <- ex_meta$Sample_source # usually a factor that indicates location

# Change this line to reflect wherever you downloaded the modified sourcetracker code to
source(
  'C:/Users/sueis/OneDrive/Documents/Teaching/AVS 454-554 DNA Sequencing Analysis Lab/R/SourceTracker_modified.r'
) #CHANGE ME
...
results <- predict(st, ex_seqtab_st[test.ix, ], alpha1 = alpha1, alpha2 =
                     alpha2)
...
meltdown$Month <- as.factor(sapply(meltdown$Date, function(x)
  strsplit(x, "/")[[1]][1]))
levels(meltdown$Month) <- c("June", "July", "August")
meltdown$Day <- as.factor(sapply(meltdown$Date, function(x)
  strsplit(x, "/")[[1]][2]))

meltdown$Rep <- as.factor(sapply(meltdown$id, function(x)
  strsplit(x, "_")[[1]][3]))
...
meltdown$SID <- as.factor(sapply(meltdown$id, function(x)
  strsplit(x, "_")[[1]][5]))
...
ggplot(meltdown, aes(x = Week, y = value, fill = Diet)) + geom_bar(stat =
                                                                     "identity") + facet_wrap(~ Downstream.Site + Month, scales = "free_x") + xlab("")
...
dist.object <- dist(
  as.data.frame(otu_table(EX_ps_clean.rar)),
  method = "euclidean",
  diag = FALSE,
  upper = FALSE,
  p = 2
